...
class F2FDataset(basedataset.BaseDataset):
    """
    This class is for intermediate fusion where each vehicle transmit the
    deep features to ego.
    """
    def __init__(self, params, visualize, train=True):
        super(F2FDataset, self). \
    ...
    def __getitem__(self, idx):
        ...
        if self.params['mode'] == 'feature':
            if self.train:
                # folder_path = f'feature_saved/train/features_{id_data}.npz'
                folder_path = f'FEATURE_SAVED/feature_saved_compressed/train/features_{id_data}.npz'
                gt_features = np.load(folder_path)
                gt_features = gt_features['features']
            else:
                # folder_path = f'feature_saved/validate/features_{id_data}.npz'
                folder_path = f'FEATURE_SAVED/feature_saved_compressed/train/features_{id_data}.npz'
                gt_features = np.load(folder_path)
                gt_features = gt_features['features']
        elif self.params['mode'] == 'no_feature':
            # gt_features = np.random.rand(cav_num, 256, 48, 176)
            gt_features = np.random.rand(cav_num, 8, 48, 176)
    ...
    def collate_batch_train(self, batch):
        ...
        try:
            gt_features, mask = regroup(gt_features, record_len, self.max_cav)
        except:
            print('wrong!!!!')
