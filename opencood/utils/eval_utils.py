...
def eval_final_results(result_stat, save_path, global_sort_detections, get_result=False, name_file=None, opt=None):
    dump_dict = {}
    ...
    with open(folder_save, 'w') as f:
        #writer details from opt
        f.write('specific_path: %s\n' % opt.specific_path)
        f.write('specific_epoch: %s\n' % opt.specific_epoch)
        f.write('split_dataset: %s\n' % opt.split_dataset)
        f.write('module_delay: %s\n' % opt.module_delay)
        f.write('mode: %s\n' % opt.mode)

        f.write('The Average Precision at IOU 0.3 is %.3f, '
                '\n'
                'The Average Precision at IOU 0.5 is %.3f, '
                '\n'
                'The Average Precision at IOU 0.7 is %.3f' % (ap_30, ap_50, ap_70))
        # f.write('\n')
        # f.write('The Average Precision at IOU 0.5 is %s' % mpre_50)
        # f.write('\n')
        # f.write('The Average Recall at IOU 0.5 is %s' % mrec_50)
        # f.write('\n')
        # f.write('The Average Precision at IOU 0.7 is %s' % mpre_70)
        # f.write('\n')
        # f.write('The Average Recall at IOU 0.7 is %s' % mrec_70)