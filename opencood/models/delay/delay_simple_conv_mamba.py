...
class FutureFramePredictor(nn.Module):
    def __init__(self, t=4, input_channels=8, height=48, width=176, projected_dim=512, patch_size=8, num_layers=3):
        ...
        self.seqential = nn.Sequential(
            ...
            torch.nn.Conv2d(input_channels, 1, kernel_size=1),
            Rearrange('(b t) 1 h w -> b t (h w)', t=self.t),
            nn.Linear(height*width, projected_dim),
            nn.Sequential(*[
                Mamba2(
            ...
            nn.Linear(projected_dim, input_channels*height*width),
            Rearrange('b t (c h w) -> b t ', nh=height//patch_size, nw=width//patch_size),
...
if __name__ == '__main__':
    ...
    patch_tokens = x_merged = einops.rearrange(x, 'b t c h w -> (b t) c h w') # torch.cat([scan_row_major, scan_col_major], dim=2)
    print(f"Patch token shape: {patch_tokens.shape}")  # [B, T, num_patches, 2*C*16*16]

    d_model = 768
    projection_1 = torch.nn.Conv2d(C, 1, kernel_size=1).to("cuda")
    projection_2 = torch.nn.Linear(H*W, d_model).to("cuda")
    x_projected = projection_1(patch_tokens)
    x_conv = einops.rearrange(x_projected, '(b t) 1 h w -> b t (h w)', t=T)
    x_projected = projection_2(x_conv)

    print(f"Projected patch token shape: {x_projected.shape}")  # [B, T, num_patches, d_model]

    #x_flattened = x_conv.flatten(start_dim=2)
