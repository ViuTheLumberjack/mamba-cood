...
class V2XTEncoder(nn.Module):
    ...
    def forward(self, x, mask, spatial_correction_matrix):
        ...
        if self.use_RTE:
            # dt: (B,L)
            #todo: work here, it should be the delay-aware positional encoding
            dt = prior_encoding[:, :, 0, 0, 1].to(torch.int)
            x = self.rte(x, dt)
        x = self.sttf(x, mask, spatial_correction_matrix)
        com_mask = mask.unsqueeze(1).unsqueeze(2).unsqueeze(3) if not self.use_roi_mask else get_roi_and_cav_mask(x.shape,
                                                                  mask,
