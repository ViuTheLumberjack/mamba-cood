...
def train_parser():
    ...
    parser.add_argument('--module_delay', action='store_true', default=True)
...
def main():
    ...
    print('Using delay module? {}'.format(opt.module_delay))

    print(hypes)
    if opt.module_delay and opt.delay is not None:
    ...
    wandb.init(project='opencood_finetune', notes="", name=info_name, save_code=True, mode=mode_wandb, config=hypes)
    ...
    epoches = hypes['train_params']['epoches'] if not opt.max_epoches else opt.max_epoches
        ...
        if first_epoch == False:
            ...
            for i, batch_data in tqdm.tqdm(enumerate(train_loader)):
                ...
                if global_iteration % 200 == 0:
                    show_pred_gt(ouput_dict["predictions"], batch_data, global_iteration, epoch, phase='train')
